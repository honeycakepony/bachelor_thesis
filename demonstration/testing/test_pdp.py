...
class TestPDP(unittest.TestCase):
    ...
    def test_sid_valid_1(self):
        ...
        response: bool = pdp_os._is_valid_sid(user_valid.id, user_valid.type, log=True)
        self.assertTrue(response)

    def test_sid_valid_2(self):
        machine_valid = Machine('r2-d2@mission−thesis.org')
        response: bool = pdp_os._is_valid_sid(machine_valid.id, machine_valid.type, log=True)
        self.assertTrue(response)

    def test_sid_invalid_1(self):
        user_invalid = User('gabrielll@mission−thesis.org')
        response: bool =  pdp_os._is_valid_sid(user_invalid.id, user_invalid.type, log=True)
    ...
    def test_sid_invalid_3(self):
        ...
        response: bool =  pdp_os._is_valid_sid(machine_invalid.id, machine_invalid.type, log=True)
    ...
    def test_stype_valid_1(self):
        ...
        response: bool =  pdp_os._is_valid_stype(user_valid.type, log=True)
        self.assertTrue(response)

    def test_stype_valid_2(self):
        machine_valid = Machine('test@mission−thesis.org')
        response: bool =  pdp_os._is_valid_stype(machine_valid.type, log=True)
        self.assertTrue(response)

    def test_stype_invalid_1(self):
        machine_invalid = Machine('test@mission−thesis.org')
        machine_invalid.type = 'auditor'
        response: bool =  pdp_os._is_valid_stype(machine_invalid.type, log=True)
        self.assertFalse(response)  # 'machine_invalid.type' invalid
    ...
    def test_stype_invalid_3(self):
        ...
        user_invalid.type = 'superuser'
        response: bool =  pdp_os._is_valid_stype(user_invalid.type, log=True)
        self.assertFalse(response) # 'user_invalid.type' invalid

    def test_check_required_params_valid_1(self):
        access_request_valid: dict = create_dummy_access_request_no_context( User('gabriel@mission−thesis.org').__make_dict__())
        response = requests.get(URL + 'check_params',
                                params={'parametrised': False, 'drop_ok': False},
                                json=access_request_valid)
    ...
    def test_check_required_params_valid_2(self):
        access_request_valid: dict = create_dummy_access_request_no_context(Machine('r2-d2@mission−thesis.org').__make_dict__())
        response = requests.get(URL + 'check_params',
                                params={'parametrised': False, 'drop_ok': False},
                                json=access_request_valid)
        self.assertEqual(response.status_code, 200)
